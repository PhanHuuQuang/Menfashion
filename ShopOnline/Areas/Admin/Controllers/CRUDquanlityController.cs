...
namespace ShopOnline.Areas.Admin.Controllers
{
    public class CRUDquanlityController : Controller
    {
        ...
        public ActionResult Delete(int? id)
        {
            try
            {
                var checkQuantity = db.Amounts.FirstOrDefault(model => model.quantityId == id);
                // Kiểm tra xem với mã Product có tồn tại trong bảng InvoinceDetail không?
                if (checkQuantity == null) // Nếu có giá trị thì xuất thông báo lỗi
                {
                ...
                {
                    db.Amounts.Remove(checkQuantity);
                    db.SaveChanges();
                    return RedirectToAction("Index");
                }
            }
